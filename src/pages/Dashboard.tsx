...
import { fmtInr, fmtPct, fmtChange, TIER } from "../lib/format";
import { EMI, EMI_PER_DAY } from "../lib/compute";
import {
  TrendingUp,
  TrendingDown,
  Shield,
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500">
          {cur.daysInPeriod}d ({dateFrom} to {dateTo}) &middot;{" "}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        {metrics.map((m) => {
          ...
          return (
            <div
              key={m.label}
              className={`rounded-xl border p-3.5 ${m.good ? "bg-slate-800/60 border-slate-700/50" : "bg-red-950/20 border-red-900/30"}`}
            >
              ...
              <div className="flex items-center gap-1.5 mt-1.5">
                {prev && change.text !== "-" && (
                  <span
                    className={`text-xs flex items-center gap-0.5 ${changePositive ? "text-green-400" : "text-red-400"}`}
                  >
                    {changePositive ? (
                      <TrendingUp size={12} />
                    ) : (
      ...
      <div className="rounded-xl border bg-slate-800/60 border-slate-700/50 p-4">
        ...
        <div className="w-full bg-slate-700/50 rounded-full h-3 overflow-hidden">
          <div
            className={`h-3 rounded-full ${cur.emiCoverage >= 1.5 ? "bg-green-500" : cur.emiCoverage >= 1 ? "bg-amber-500" : "bg-red-500"}`}
            style={{ width: `${Math.min(100, (cur.emiCoverage / 2) * 100)}%` }}
          />
        </div>
        <div className="flex justify-between text-[10px] text-slate-600 mt-1">
          <span>0x</span>
          <span className="text-amber-500">1.0x EMI</span>
          <span className="text-green-500">1.5x</span>
          <span>2.0x</span>
        </div>
